...
const AutoComplete: React.FC<AutoCompleteProps> = ({ onClear, options, onPlaceSelect, value, className, placeholder }) => {
    ...
    return <div className="relative w-full">
        <input ref={inputRef} onChange={(e) => { console.log("+=====", e.target.value); }} placeholder={placeholder} className={"border  outline-none px-3 py-2 w-full h-[42px] focus:border-blue-300  transition-all " + className} />
        {
...
export function TimePicker({ className, onChange, selectedTime, placeholder }: TimePickerProps) {
    ...
    return (
        <select
            className={`border outline-none px-3 h-[42px] focus:border-blue-300 transition-all  w-full ${className}`}
...
export function InputBox({ readOnly = false, type = "text", className, onChange, placeholder, value }: { type?: string, placeholder?: string, value: string | number | readonly string[] | undefined, readOnly?: boolean, onChange: (e: string) => void, className?: string }) {
    ...
    return <div className="relative w-full">
        <input readOnly={readOnly} type={type} onChange={(e) => onChange(e.target.value)} value={value} placeholder={placeholder} className={"border outline-none px-3 py-2 w-full h-[42px] focus:border-blue-300 transition-all " + className} />
        {
            value ? <div onClick={() => {
                onChange("")
