...
function Blogs() {
    return (
        <div className="pt-28  pb-20 bg-blue-50">

            <ContainerWrapper>
                <div className="flex w-full flex-col justify-center items-center text-center gap-5">
                    <h1 className="font-bold text-blue-900  text-2xl">OUR BLOG</h1>
...
function BlogBox({ date, link, name, title, image }: { image: string, name: string, title: string, date: string, link: string }) {
    return (
        <>
            <div className="shadow-xl shadow-gray-200  bg-white p-4  flex flex-col gap-3 pb-5 mt-32 transition-transform duration-500 transform hover:-translate-y-2">
                ...
                <div className="flex w-full justify-between items-center border-b pb-3">
                    <div className="font-bold text-gray-800 flex justify-center items-center gap-3"><FaRegUserCircle size={20} className="text-blue-900" /> By {name}</div>
                    <div className="font-bold text-gray-800 flex justify-center items-center gap-3"><BsCalendar2Date size={20} className="text-blue-900" /> {date}</div>

                </div>
                <h1 className="font-bold text-2xl">{title}</h1>
                <a href={link} className="h-12 font-bold w-40 mt-2 gap-4 bg-blue-900 hover:bg-gray-950 transition-all flex justify-center  items-center text-white">Read More <FaArrowRightLong /></a>
            </div>
